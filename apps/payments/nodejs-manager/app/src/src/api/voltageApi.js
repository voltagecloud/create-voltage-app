 ...
 * @param {string} walletId - ID of the wallet to create the address for
 * @param {number} amountSats - Amount in satoshis
 * @param {string} description - Optional description for the payment
 * @returns {Promise<Object>} - Promise resolving to the created payment object
 */
async function createOnchainAddress(walletId, amountSats, description = "") {
  try {
    const url = `${config.baseUrl}/organizations/${config.organizationId}/environments/${config.environmentId}/payments`;
    const paymentData = {
      id: crypto.randomUUID(),
    ...
    console.error("Error creating onchain address:", error.message);
    throw error;
  }
}
